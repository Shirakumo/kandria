...
(defun launch (&rest initargs)
  #+nx (setf (org.shirakumo.fraf.trial.nxgl:performance-mode) :loading)
  (let ((*package* #.*package*))
...
(defmethod load-game (state (main main))
  (#+nx org.shirakumo.fraf.trial.nxgl:with-performance-mode
   #-nx progn :loading
   (let ((scene (scene main)))
...
(defmethod setup-rendering :after ((main main))
  ...
  (etypecase (state main)
    ...
    (null (if (setting :debugging :show-startup-screen)
              ...
              (show-panel 'main-menu))))
  #+nx (setf (org.shirakumo.fraf.trial.nxgl:performance-mode) :normal))
