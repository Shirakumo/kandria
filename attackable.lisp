...
(defmethod handle-attack-states ((attackable attackable) ev)
  (case (state attackable)
    (:attacking
     (let ((acc (acceleration attackable)))
       (nv* acc 0)
       (let ((frame (frame-data attackable))
             (hurtbox (hurtbox attackable)))
         (incf (vx acc) (* (direction attackable) (vx (frame-velocity frame))))
         (incf (vy acc) (vy (frame-velocity frame)))
         (let ((other (scan (surface attackable) hurtbox)))
           (when (typep other 'enemy)
             (incf (vx (acceleration other)) (* (direction attackable) (vx (frame-velocity frame))))
             (incf (vy (acceleration other)) (vy (frame-velocity frame)))
