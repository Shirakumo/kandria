...
(defmethod paint-tile ((tool paint) event)
  (let* ((entity (entity tool))
    ...
    (cond ((retained :control)
           (print (layer (sidebar (editor tool))))
           (cond ((or (show-solids entity) (= +base-layer+ (layer (sidebar (editor tool)))))
                 ...
                 ((= 0 (layer (sidebar (editor tool))))
                  (let* ((base-layer (aref (layers entity) 0))
                    ...
                    (with-cleanup-on-failure (progn (setf (pixel-data base-layer) layer))
                      (with-commit (tool)
                        ((%auto-tile-bg (pixel-data base-layer) (truncate (vx (size base-layer))) (truncate (vy (size base-layer)))
