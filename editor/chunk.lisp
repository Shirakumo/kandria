...
(defmethod initialize-instance :after ((structure tile-picker) &key widget)
  (let* ((tileset (albedo (entity widget)))
    ...
    (dotimes (y (floor (height tileset) +tile-size+))
      (dotimes (x (floor (width tileset) +tile-size+))
        (let* ((tile (list x (- (floor (height tileset) +tile-size+) y 1) 1 1))
          ...
          (alloy:on alloy:activate (element)
            (if (retained :shift)
                (let* ((x (first (tile-to-place widget)))
                  ...
                  (setf (nth 0 (tile-to-place widget)) (min x (first tile)))
                  (setf (nth 1 (tile-to-place widget)) (min y (second tile)))
                  (setf (nth 2 (tile-to-place widget)) xd)
                  (setf (nth 3 (tile-to-place widget)) yd)
                  (setf (tile-to-place widget) (tile-to-place widget)))
                (progn
