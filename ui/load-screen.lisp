...
(defmethod trial:commit (object (loader load-screen) &rest args &key unload show-screen cold)
  (cond ((trial::current-area loader)
         (stage object (trial::current-area loader)))
        (show-screen
         (let ((*show-load-screen* T)
               (area (make-instance 'staging-area :loader loader)))
    ...
    (let ((prog (if (= so-far total) 100.0 (* 0.1 (floor (* 1000 (/ so-far 14000))))))
      ...
      (when (and (/= prog (slot-value loader 'progress))
        ...
        (setf (alloy:value (text (find-panel 'load-panel)))
              (subseq text 0 (min (length text) (floor (* 0.01 prog (length text))))))
        (render ui T)
        (blit-to-screen ui)
