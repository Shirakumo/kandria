  ...
  (let ((layout (make-instance 'menu-layout))
    ...
    (macrolet ((with-tab ((name layout &optional (focus ''vertical-menu-focus-list)) &body body)
                 `(let* ((layout (make-instance ,layout))
                    ...
                    ,@body
                    (add-tab tabs (make-instance 'trial-alloy:language-data :name ,name) layout focus)))
               (with-tab-view (name &body body)
                 `(let* ((view (make-instance 'tab-view))
                         (tab (add-tab tabs ,(alloy:expand-place-data name) (alloy:layout-element view) (alloy:focus-element view)))
                         (tabs view))
                    (declare (ignore tab))
                    ,@body))
      ...
      (let ((view (make-instance 'options-menu)))
        (add-tab tabs (make-instance 'trial-alloy:language-data :name 'open-options-menu) (alloy:layout-element view) (alloy:focus-element view)))
