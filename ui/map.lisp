...
(defmethod presentations:realize-renderable ((renderer presentations:renderer) (map map-element))
  ...
  (let ((array (make-array 0 :adjustable T :fill-pointer T))
    ...
    (flet ((unit-marker (unit color)
           ...
           (target-marker (location size)
             (let ((bounds (alloy:extent (- (vx location) (/ size 2))
                                         (- (vy location) (/ size 2))
                                         size size))
                   (color (colored:color 1 0.1 0.1 0.25)))
               (vector-push-extend (cons :target (simple:ellipse renderer bounds :pattern color :name :target :z-index 1)) array)
               (vector-push-extend (cons :target (simple:ellipse renderer bounds :pattern colors:red :line-width (alloy:un 4) :name :target :z-index 1)) array))))
        ...
        (typecase unit
          ...
          (npc
           ...
           (when (and (eql :lead (ai-state unit))
             ...
             (target-marker (location unit) (* 40 +tile-size+))))))
      (dolist (quest (quest:known-quests (storyline +world+)))
        (dolist (task (quest:active-tasks quest))
          (when (marker task)
            (destructuring-bind (location size) (enlist (marker task) (* 40 +tile-size+))
              (target-marker (ensure-location location) size)))))
