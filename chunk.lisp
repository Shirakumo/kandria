...
(defun find-ground (layer location)
  (let ((w (truncate (vx (size layer))))
    ...
    (%with-layer-xy (layer location)
      (let ((ground (loop for i downfrom (* 2 (+ x (* y w))) to 0 by (* 2 w)
                          do (when (< 0 (aref data i) 16)
                               (return i))
                          finally (return (* 2 (+ x (* y w)))))))
        (loop for i from ground below (length data) by (* 2 w)
              do (when (= 0 (aref data i))
