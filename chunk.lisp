...
(defmethod scan ((chunk chunk) (target vec4) on-hit)
  ...
  (let* ((tilemap (pixel-data chunk))
         (t-s (the (unsigned-byte 16) +tile-size+))
         (w (truncate (vx (the vec2 (size chunk)))))
         (h (truncate (vy (the vec2 (size chunk)))))
         (location (location chunk))
         (bsize (bsize chunk))
         (x (+ (- (vx target) (vx location)) (vx bsize)))
         (y (+ (- (vy target) (vy location)) (vy bsize)))
    ...
    (declare (type (unsigned-byte 32) x- x+ y- y+))
    (declare (type (unsigned-byte 16) w h))
    (declare (type single-float x y))
    (declare (type vec2 location bsize))
    ...
    (loop for x from (max 0 x-) below (min w x+)
          do (loop for y from (max 0 y-) below (min h y+)
                   ...
                   do (when (< 0 tile 17)
                        (let* ((loc (vec2 (+ (* x t-s) (truncate t-s 2) (- (vx location) (vx bsize)))
                                          (+ (* y t-s) (truncate t-s 2) (- (vy location) (vy bsize)))))
