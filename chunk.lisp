...
(define-shader-entity chunk (layered-container sized-entity lit-entity solid)
...
(defmethod scan ((chunk chunk) (target vec4))
  (let* ((tilemap (aref (layers chunk) +layer-count+))
         ...
         (x- (floor (+ (- (vx target) (vz target) (vx2 loc)) (vx2 bs)) +tile-size+))
         (x+ (ceiling (+ (+ (vx target) (vz target) (vx2 loc)) (vx2 bs)) +tile-size+))
         (y- (floor (+ (- (vy target) (vw target) (vy2 loc)) (vy2 bs)) +tile-size+))
         (y+ (ceiling (+ (+ (vy target) (vw target) (vy2 loc)) (vy2 bs)) +tile-size+)))
