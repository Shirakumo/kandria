...
(defmethod refresh-language ((all (eql T)))
  ...
  (when (= 0 (hash-table-count (default-interactions (storyline +world+))))
    (load-default-interactions :table (default-interactions (storyline +world+))))
    ...
    (when (typep entity 'profile)
      (unless (equal (nametag entity) (@ unknown-nametag))
        (setf (nametag entity) (or (language-string (intern (format NIL "~a-~a" (string (type-of entity)) 'nametag) (symbol-package (type-of entity))) NIL)
                                   (nametag entity)))))
    (when (typep entity 'dialog-entity)
      (setf (default-interaction entity) (default-interaction-init entity)))))
