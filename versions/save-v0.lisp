    ...
    (destructuring-bind (&key region (clock 0.0) (timestamp (initial-timestamp)) &allow-other-keys) world-data
      ...
      (let* ((region (or (region world) (load-region T world)))
             (initargs (handler-case (first (parse-sexps (depot:read-from (depot:entry* depot "regions" (format NIL "~(~a~).lisp" (name region))) 'character)))
                         (error (e)
                           (v:warn :kandria.save "Failed to load region initargs: ~a" e)))))
...
(define-encoder (world save-v2) (_b depot)
  ...
  (flet ((encode (object &optional (_b _b))
           (encode-payload object _b depot save-v2)))
    (encode (storyline world))
...
(define-decoder (world save-v2) (_b depot)
  ...
  (flet ((decode (object &optional (_b _b))
           (decode-payload _b (if (symbolp object) (type-prototype object) object) depot save-v2)))
