  ...
  (let* ((collisions (collisions player))
    ...
    (ecase (state player)
       ...
       (or (and (< (dash-time player) (p! dash-evade-grace-time))
           ...
           (cond ((< (p! dash-min-time) (dash-time player))
                 ...
                 (T
                  ...
                  (vsetf vel 0 0)
                  (when (< (p! dash-acc-start) (+ (dash-time player) (dt ev)))
                    (vsetf vel
      ...
      (:climbing
       ...
       (setf (air-time player) 0.0)
       ;; Movement
       (let* ((offset (tvec (+ (vx loc) (* (direction player) (+ 10 (vx size)))) (- (vy loc) (vy size) 2)))
              (top (scan-collision +world+ offset))
              (ledge (scan-collision +world+ (tv+ offset #.(vec 0 16))))
         ...
         (cond ((retained 'jump)
                ...
                (harmony:stop (// 'sound 'rope-slide-down))
                (setf (state player) :normal))
               ((or (and top (eq attached top))
                    (null ledge)
                    (not (typep (hit-object ledge) 'ground)))
