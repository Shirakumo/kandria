  ...
  (let* ((collisions (collisions player))
    ...
    (ecase (state player)
      ...
      (:dashing
       ...
       ;; Handle crawlspaces
       (cond ((< 0 (vx vel))
              (when (and (typep (svref collisions 1) 'ground)
                         (null (scan-collision +world+ (tvec (+ (vx loc) (vx size) 8) (- (vy loc) 4))))
                         (scan-collision +world+ (tvec (+ (vx loc) (vx size) 8) (+ (vy loc) 4)))
                         (scan-collision +world+ (tvec (+ (vx loc) (vx size) 8) (- (vy loc) (vy size) 4))))
                (setf (vw (color player)) 0.0)
                (setf (vx vel) (vx (p! velocity-limit)))
                (setf (state player) :crawling)))
             ((< (vx vel) 0)
              (when (and (typep (svref collisions 3) 'ground)
                         (null (scan-collision +world+ (tvec (- (vx loc) (vx size) 8) (- (vy loc) 4))))
                         (scan-collision +world+ (tvec (- (vx loc) (vx size) 8) (+ (vy loc) 4)))
                         (scan-collision +world+ (tvec (- (vx loc) (vx size) 8) (- (vy loc) (vy size) 4))))
                (setf (vw (color player)) 0.0)
