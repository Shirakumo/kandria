...
#-kandria-release
(let ((type (copy-seq '(dummy zombie rogue))))
...
(defmethod handle :before ((ev tick) (player player))
  ...
  (let* ((collisions (collisions player))
    ...
    (when (and (v= 0 vel)
      ...
      (bvh:do-fitting (entity (bvh (region +world+)) (tvec (- (vx loc) (* +tile-size+ 13))
                                                           (- (vy loc) (* +tile-size+ 10))
                                                           (+ (vx loc) (* +tile-size+ 13))
                                                           (vy loc)))
        (when (and (typep entity 'water) (< (vy (location entity)) (vy (location player))))
          (let ((dx (/ (- (vx (location entity)) (vx loc)) 2.0)))
            (when (and (null (scan-collision-for player +world+ (tvec (+ (vx loc) dx) (+ (vy loc) 16) (abs dx) (vy size))))
                       (interactable-p entity))
              (setf (interactable player) entity)
              (return))))))
