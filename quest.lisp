...
(defmethod dialogue:wrap-lexenv ((assembly assembly) form)
  `(let* ((world +world+)
     ...
     (labels ((thing (thing)
              ...
              (activate (thing)
                (quest:activate (thing thing)))
              (deactivate (thing)
                (quest:deactivate (thing thing)))
              (complete (thing)
                (quest:complete (thing thing)))
              (fail (thing)
                (quest:fail (thing thing)))
              (active-p (thing)
                (quest:active-p (thing thing)))
              (complete-p (thing)
                (eql :complete (quest:status (thing thing))))
              (failed-p (thing)
                (eql :failed (quest:status (thing thing))))
